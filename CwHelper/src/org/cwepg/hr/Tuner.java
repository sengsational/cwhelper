...
public abstract class Tuner { //implements Comparable {
	...
	public boolean available(Capture capture, boolean mustBeInLineup) {
        if (capture == null){
            System.out.println(new Date() + " Tuner.available() Capture null.");
            return false;
        }
        if (!slotOpen(capture.slot)) {
            System.out.println(new Date() + " Tuner.available() Slot not open.");
            return false;
        }
        if (mustBeInLineup && !this.lineUp.contains(capture.channel)){
            System.out.println(new Date() + " Tuner.available() Channel not in lineup.");
            return false;
        }
		return true;
	}
    ...
    public Calendar getNextCaptureCalendar(){
    	...
    	if (captureWithNextEvent != null){
    		return captureWithNextEvent.getUnhandledEvent();
    	}
    	return null;
    }
		...
		for (Iterator iter = captures.iterator(); iter.hasNext();) {
			...
			if (unhandledEvent != null){
				if (nextEventCalendar == null || unhandledEvent.before(nextEventCalendar)){
					...
					// System.out.println("capture did not have next event " + capture);
				}
			}
		}
		return nextCapture;
    ...
    public void delistCapturesMatchingFileName(String fileName, ArrayList<Capture> captures) {
        ArrayList<Capture> toRemove = new ArrayList<Capture>();
        for (int i = 0; i < captures.size(); i++){
            Capture aCapture = captures.get(i);
            if (aCapture.getFileName().equals(fileName)){
