...
public class Target {
    ...
    public int setNextAvailablePort() {
        this.port = PortManager.getNextAvailablePort();
		return this.port;
	}
    ...
    public static String getNoPathNoExtensionFilename(String pathFile){
        ...
        if (dotLoc < 1 || (fn.length() - dotLoc) > 4 || fn.endsWith(".")) return fn;  //no extension or long extention that's not really an extension
        else return fn.substring(0,dotLoc);
    }
