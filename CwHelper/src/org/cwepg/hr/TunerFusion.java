...
public class TunerFusion extends Tuner {
    ...
    private static final SimpleDateFormat DBDF = new SimpleDateFormat("MM/dd/yyyy");
    private static final SimpleDateFormat DBTF = new SimpleDateFormat("HH:mm");
    ...
    private List<Capture> getCapturesFromFile(LineUp lineUp) {
        ...
        try {
            ...
            while (rs.next()){
                ...
                Target target = null;
                if (Target.getNoPathNoExtensionFilename(fileNameNoPathNoExtension).equals(fileNameNoPathNoExtension)){
                ...
                for (Iterator iter = slots.iterator(); iter.hasNext();) {
                    ...
                    capture.setFusionData(gCode, "" + devId, isRecord, attribute, fileNameNoPathNoExtension, etcStr);message+="12 ";
                    if(capture.getLastError() != null) {
                        capture = null;
                        throw new Exception(capture.getLastError());
                    }
    ...
    public void removeLocalCapture(Capture aCapture) {
        ...
        if (aCapture.isRecurring()){
            delistCapturesMatchingFileName(aCapture.getFileName(), captures);
        } else {
    ...
    public void removeCapture(int j) {
        ...
        removeCaptureFromOwningStore(aCapture); 
        if (aCapture.isRecurring()){
            delistCapturesMatchingFileName(aCapture.getFileName(), captures);
        } else {
            aCapture.removeWakeup();
        ...
        String result =  CaptureFusion.notifyExternalApplication(true); // true means use command line instead of interprocess communication
    ...
    void removeCaptureFromOwningStore(Capture capture) {
        Connection connection = null;
        Statement statement = null;
        ResultSet rs = null;
        try {
            ...
            int count = 0;
            if (capture != null) {
                deleteSql = getDeleteSql(capture.slot, capture.target.getNoPathNoExtensionFilename());
                if (((CaptureFusion)capture).isRecurring){
                ...
                if (countRs.next()){
                    ...
                    if (foundCount > 0){
                        ...
                        deleteSql = "delete from ReserveList where ReserveChName = 'D99-99'";
    ...
    private String getDeleteSql(Slot slot, String dbFilename) {
        ...
        buf.append("where Start_Time = " + toDateFormattingStart + DBDTFS.format(slot.start.getTime()) + toDateFormattingEnd);
    ...
    public static void main(String[] args) throws Exception {
        boolean testGetFusionTuners = true;
