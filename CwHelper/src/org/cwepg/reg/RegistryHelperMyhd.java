...
public class RegistryHelperMyhd {
    ...
    private static String getReservationNameForFileName(Map map, String fileName, boolean b) {
        threeDigit.setMinimumIntegerDigits(3);
        String reservationName = null;
        try {
            byte[] buf = null;
    ...
    public static void deleteFromSchedule(String fileName) throws Exception {
        Map map = Registry.getValues(topKey, myhdBranch);
        String scheduleToDelete = getReservationNameForFileName(map, fileName, true);
        if (scheduleToDelete == null) throw new Exception("There is no matching reservation for " + fileName);
        deleteReservation(scheduleToDelete, map);
    }
